...
latin = find(lat <= 7.5 & lat >= 5);
lonin = find(lon >= 140 & lon <= 160);
lonSurf = nc_varget(ERAHFX,'longitude');
latSurf = nc_varget(ERAHFX,'latitude');
latinVert = latin;
latinSurf=find(latSurf <= 7.5 & latSurf >= 5);
latinHorz = [latinVert(1)-1; latinVert; latinVert(end)+1];
loninVert = lonin;
loninSurf=find(lonSurf <= 160 & lonSurf >= 140);
...
precip_ls = mean(mean(precip_ls(:,latinSurf,loninSurf)*86400,2),3);
precip_total=nc_varget(ERAprecip,'mtpr');
precip_total = mean(mean(precip_total(:,latinSurf,loninSurf)*86400,2),3);
precip_conv=nc_varget(ERAprecip,'mcpr');
precip_conv = mean(mean(precip_conv(:,latinSurf,loninSurf)*86400,2),3);
evap=nc_varget(ERAprecip,'mer');
evap = -mean(mean(evap(:,latinSurf,loninSurf)*86400,2),3);
...
% ylabel('Pressure (pa)');
% title('Omega ds/dp')
int_S = trapz(pressureshort',-(dSdp.*OmegaShort)'/g);
int_Sh = trapz(pressure(11:end)',-(dSdu(:,11:end).*U(:,11:end)+dSdv(:,11:end).*V(:,11:end))'/g);
int_H= trapz(pressureshort(11:end)',-(dHdp(:,11:end).*OmegaShort(:,11:end))'/g);
int_Hh = trapz(pressure(11:end)',-(dHdu(:,11:end).*U(:,11:end)+dHdv(:,11:end).*V(:,11:end))'/g);
...
figure(2),
hold on;
scatter(SST_box,II,108,anglecolors,'o','filled');
hold on;
scatter(mean(SST_box),mean(II),324,mean_color,'x');
lgd4=legend('EP monthly mean','EP climatology','WP monthly mean','WP climatology','position',[.173 .166 .224 .138]);
set(gca,'fontsize',16);
saveas(gcf,'../PaperPlots/II_sst_ERA5.png');
