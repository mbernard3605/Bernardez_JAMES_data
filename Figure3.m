...
ERARad = 'radiation_new.nc';
ERAHFX = 'flux.nc';
ERASurf = 'sst.nc';
ERAVert = 'Omega.nc';
ERAprecip= 'rain.nc';
landFile = 'ERA5_land.nc';
...
close all;
figure,
set(gcf,'units','normalized','outerposition',[0.075 0.25 0.91 0.71]);

ax(2)=subplot(2,1,1);
[c,h]=contourf([lon(length(lon)/2+1:end); lon(1:length(lon)/2)+360],lat,[SST_mean(:,length(lon)/2+1:end) SST_mean(:,1:length(lon)/2)],[280 296:303]);
axis equal;
...
ax(4)=subplot(2,1,2);
[c,h]=contourf([x(length(x(2:end-1,1))/2+1:end-1,1); x(2:length(x(2:end-1,1))/2,1)+360],y(1,2:end-1),-[sst_grad_smooth(:,length(x(2:end-1,1))/2+1:end) sst_grad_smooth(:,1:length(x(2:end-1,1))/2)],[linspace(-1e-11,-2.5e-12,4) -1e-13 1e-13 linspace(2.5e-12,1e-11,4)]);
axis equal;
set(h,'linecolor','none');set(gca,'fontsize',20);
caxis([-1e-11 1e-11]);
hold on;
contour([lon(length(lon)/2:end-1); lon(1:length(lon)/2)+360],lat,[sst_land(:,length(lon)/2+1:end) sst_land(:,1:length(lon)/2)],[1 1],'k','linewidth',2);
set(gca,'ylim',[-20 20],'xlim',[0 360]);
title('Laplacian of SST [K/m^2]');
contour(Meshx,Meshy,eastPlotBox,[1 1],'k','linewidth',2);
contour(Meshx,Meshy,westPlotBox,[1 1],'k','linewidth',2);
colormap(ax(4),redblue);cbh=colorbar('xtick',[-1e-11 0 1e-11],'xticklabel',[{'1e-11'  };{'0'   }; {'-1e-11' }]);
