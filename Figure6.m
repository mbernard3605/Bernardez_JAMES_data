...
dynNames={};
 ...
 dynPlot=[plotDirectory ''];
 dynFiles = [dir([allDirectory 'wrfout_e*']);dir([allDirectory 'wrfout_w*'])];
 dynFiles([1,7])=[];
 for i = 1:length(dynFiles)
 dynNames{i}=dynFiles(i).name;
 end
 
dynstats=simple_wtg_plot_tropolimit([allDirectory],dynNames,dynPlot,1,0);
w1 = dynstats.omega{1};
tp1 = dynstats.tp{1};
hp1 = dynstats.hp{1};
h1 = dynstats.h{1};
...
z4w=Hp4w>=HSAT4w;Tdiff4w=Tp4w-Temp4w;
    ...
    xlabel('Buoyancy Difference');title('moisture difference included','fontsize',16)
...
z4w=Hp4w>=HSAT4w;Tdiff4w=Tp4w-Temp4w;
    ...
    plot(Tdiff1e-Tdiff1w,p,'k--',-Tdiff2e+Tdiff2w,p,'ko-',-Tdiff3e+Tdiff3w,p,'k:',-Tdiff4e+Tdiff4w,p,'k-.','linewidth',3)
    set(gca,'ylim',[10000 100000],'ydir','reverse','xlim',[-2 2],'fontsize',24,'yticklabel',[],'xdir','reverse');
    xlabel('Buoyancy Difference');title('no moisture included','fontsize',16);
    sgtitle('Vertical Motion and Buoyancy Sensitivity','fontsize',24);