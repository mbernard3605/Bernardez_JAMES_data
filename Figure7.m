...
dynamoFiles = dir([allDirectory 'wrfout_Dy*']);

 dynamofullname=[allDirectory dynamoFiles(1).name];
...
%1-ii,2-DCIN,3-SF,4-gms,5-angle_mean,6-rr,7-hfx
 ...
 dynamoTherms(i,:)=calc_thermodynamics([allDirectory dynamoFiles(i).name],1);
 end
...
z_start=mean(nc_varget([allDirectory dynamoFiles(i).name],'Z_FORCE'));
 ...
 dynamofullname=[allDirectory dynamoFiles(i).name];
...
% cwv(i)=trapz(p(i,:)',-qv(i,:)'/9.81); %units in mm
 ...
 [~,~,~,~,tp1temp] = zero_plume_buoyancy_multi_plume(T(i,:),qv(i,:),z(i,:),p(i,:),4000);%,w(i,:)); 
...
subplot(1,4,1)
plot(th_start(1,:)-th_start(1,:),p_start/100,'linewidth',3)
hold on;
plot(th_start(2:3,:)-th_comp,p_start/100,'linewidth',3)
...
subplot(1,4,3),
plot(RH*100,p(1,:)/100,'linewidth',3)
set(gca,'fontsize',16,'linewidth',3)
% yline(11075,'--');
% yline(6795,'--');
% yline(500,'--');
...
subplot(1,4,4)
plot(tp1-T,p(1,:)/100,'linewidth',3)
set(gca,'fontsize',16,'linewidth',3)
% yline(11075,'--');
% yline(6795,'--');
% yline(500,'--');
ylim([100 1000]);
set(gca,'ylim',[100 1000],'ydir','reverse','xdir','reverse')
xlim([-5 3]);
